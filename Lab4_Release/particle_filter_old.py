...
def motion_update(particles, odom):
    ...
    for particle in particles:
        ...
        nx, ny, nh = add_odometry_noise(
            (x+c, y+d, h+dh), heading_sigma=setting.ODOM_HEAD_SIGMA, trans_sigma=setting.ODOM_TRANS_SIGMA)
...
def measurement_update(particles, measured_marker_list, grid):
        ...
        for particle in particles:
            ...
            if grid.is_in(x, y) and grid.is_free(x, y):
                ...
                while len(markers_visible_to_particle) > 0 and len(markers_visible_to_robot) > 0:
                    all_pairs = product(
                        markers_visible_to_particle, markers_visible_to_robot)
                    pm, rm = min(all_pairs, key=lambda p: grid_distance(
                ...
                for pm, rm in marker_pairs:
                    ...
                    prob *= max(likelihood, setting.DETECTION_FAILURE_RATE *
                                setting.SPURIOUS_DETECTION_RATE)

                # In this case, likelihood is automatically 0, and max(0, DETECTION_FAILURE_RATE) = DETECTION_FAILURE_RATE
                prob *= (setting.DETECTION_FAILURE_RATE **
                         len(markers_visible_to_particle))
                # Probability for the extra robot observation to all be spurious
                prob *= (setting.SPURIOUS_DETECTION_RATE **
                         len(markers_visible_to_robot))
                weight.append(prob)
    ...
    if norm != 0:
        ...
        measured_particles += np.random.choice(
            particles, setting.PARTICLE_COUNT-num_random_sample, p=weight).tolist()
    else:
